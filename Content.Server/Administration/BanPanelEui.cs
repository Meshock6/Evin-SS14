...
public sealed class BanPanelEui : BaseEui
{
    ...
    private async void BanPlayer(string? target, string? ipAddressString, bool useLastIp, ImmutableTypedHwid? hwid, bool useLastHwid, uint minutes, NoteSeverity severity, string reason, IReadOnlyCollection<string>? roles, bool erase)
    {
        ...
        if (roles?.Count > 0)
        {
            ...
            _DiscordWebhookBanSender.SendRoleBansMessage(target, targetUid, Player.Name, Player.UserId, minutes, reason, roles); // Evin Discord Webhook
        ...
        _DiscordWebhookBanSender.SendBanMessage(target, targetUid, Player.Name, Player.UserId, minutes, reason); // Evin Discord Webhook
