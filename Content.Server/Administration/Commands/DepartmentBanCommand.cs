...
public sealed class DepartmentBanCommand : IConsoleCommand
{
    ...
    public async void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        List<string> jobList = new List<string>(); // EvinDiscord Webhook
        ...
        foreach (var job in departmentProto.Roles)
        {
            ...
            jobList.Add(job); // Evin Discord Webhook
        }

        _DiscordWebhookBanSender.SendRoleBansMessage(target, targetUid, shell.Player?.Name, shell.Player?.UserId, minutes, reason, jobList); // Evin Discord Webhook
    }
